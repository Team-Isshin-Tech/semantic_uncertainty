...
DEFAULT_ENTAILMENT_MODEL = os.getenv(
    "SE_ENTAILMENT_MODEL",
    "microsoft/deberta-v3-small-mnli"
)
...
class EntailmentDeberta(BaseEntailment):
    def __init__(self):
        logging.info("Entailment model loading: %s", DEFAULT_ENTAILMENT_MODEL)
        self.tokenizer = AutoTokenizer.from_pretrained(DEFAULT_ENTAILMENT_MODEL, use_fast=False)
        self.model = AutoModelForSequenceClassification.from_pretrained(
            DEFAULT_ENTAILMENT_MODEL).to(DEVICE)
        logging.info("Entailment model ready on %s", DEVICE)
...
def get_semantic_ids(strings_list, model, strict_entailment=False, example=None):
    ...
    logging.info("Entailment clustering started: %d responses", len(strings_list))
    ...
    logging.info("Entailment clustering complete: %d clusters", len(set(semantic_set_ids)))
