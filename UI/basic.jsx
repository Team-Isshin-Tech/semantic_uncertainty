...
// Thresholds are bound to the model definition
const MODELS = [
...
/**
 * MOCK DATA GENERATOR
 * Handles variable N (Number of generated answers)
 */
const mockAnalysis = (question, t, d, n) => {
  // Simulating slightly different SE based on sample size
  const seValue = n === 100 ? 1.42 : 1.35; 
  const factor = n / 10;
  ...
  return {
    ...
    clusters: [
      {
        id: 1,
        representative: "The capital of France is Paris.",
        count: 7 * factor,
        color: "blue",
        variations: [
          "It's Paris.", "Paris is the city.", "France's capital city is Paris."
        ]
      },
      {
        id: 2,
        representative: "I'm not entirely sure, maybe Lyon?",
        count: 2 * factor,
        color: "amber",
        variations: ["Lyon might be the one."]
      },
      {
        id: 3,
        representative: "Marseille is the capital.",
        count: 1 * factor,
        color: "red",
        variations: []
      }
...
const App = () => {
  ...
  const handleAnalyze = () => {
    if (!selectedModel.enabled) return;
    setIsAnalyzing(true);
    const delay = sampleSize === 100 ? 3500 : 1500;
    setTimeout(() => {
      setData(mockAnalysis(question, selectedModel.t, selectedModel.d, sampleSize));
  ...
  const status = useMemo(() => {
    ...
    else if (seValue > (threshold + delta)) {
      return { 
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <header className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-3xl font-black text-slate-900 tracking-tight">Semantic Entropy <span className="text-indigo-600">Lab</span></h1>
          </div>
          
          <div className="flex flex-col items-end gap-2">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mr-1">Analysis Depth (Sample Count)</span>
            <div className="bg-white p-1 rounded-xl shadow-sm border border-slate-200 flex items-center">
              <button 
                onClick={() => setSampleSize(10)}
                className={`flex flex-col items-center px-4 py-1 rounded-lg transition-all ${sampleSize === 10 ? 'bg-slate-900 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-50'}`}
              >
              ...
              <button 
                ...
                className={`flex flex-col items-center px-4 py-1 rounded-lg transition-all ${sampleSize === 100 ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-50'}`}
              >
                <div className="flex items-center gap-2 text-xs font-bold">
        ...
        <section className="mb-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-bold text-slate-500 uppercase tracking-widest">Target Architecture Calibration</h3>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            {MODELS.map((model) => (
              ...
              >
                <div>
                  <span className={`text-[11px] font-bold leading-tight block mb-2 h-8 ${selectedModelId === model.id ? 'text-indigo-800' : 'text-slate-600'}`}>
                    {model.name}
                  </span>
                  
                  {/* Larger Threshold Display */}
                  <div className="bg-white/50 rounded-lg p-2 border border-slate-100">
                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-tighter block mb-0.5">Threshold (t ± Δ)</span>
                    <div className={`text-base font-mono font-black ${selectedModelId === model.id ? 'text-indigo-600' : 'text-slate-500'}`}>
                ...
                <div className="flex items-center justify-between mt-2">
                  {!model.enabled ? (
                    ...
                    <span className="text-[9px] bg-indigo-600 px-1.5 py-0.5 rounded text-white font-bold uppercase w-fit tracking-tighter shadow-sm">Active</span>
                  )}
                </div>
              </button>
            ))}
        ...
        <div className="relative mb-10">
          <input 
            ...
            placeholder={`Enter query for ${selectedModel.name}...`}
            value={question}
          ...
          <button 
            ...
            className={`absolute right-4 top-1/2 -translate-y-1/2 px-8 py-3 rounded-2xl font-bold transition-all disabled:opacity-30 ${sampleSize === 100 ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-slate-900 hover:bg-black'} text-white shadow-lg`}
          >
        ...
        {isAnalyzing && (
          <div className="py-20 flex flex-col items-center">
            <div className={`w-16 h-16 border-4 border-slate-100 ${sampleSize === 100 ? 'border-t-indigo-600' : 'border-t-slate-900'} rounded-full animate-spin mb-4`}></div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in duration-500">
            ...
            <div className="lg:col-span-4 space-y-6">
              ...
              <div className="bg-white p-8 rounded-[2rem] shadow-xl shadow-slate-200/50 border border-white flex flex-col items-center relative overflow-hidden">
                ...
                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mt-4 mb-8">Entropy Meter</h3>
                ...
                <div className="relative w-48 h-24 mb-6 overflow-hidden">
                  ...
                  <div 
                    ...
                    style={{ transform: `translateX(-50%) rotate(${(Math.min(data.seValue, 3) / 3 * 180) - 90}deg)` }}
                  >
            ...
            <div className="lg:col-span-8 space-y-4">
              <div className="flex items-center justify-between mb-4 px-2">
                ...
                <div className="flex items-center gap-2">
                  <span className={`w-2 h-2 rounded-full ${data.sampleSize === 100 ? 'bg-indigo-500 animate-pulse' : 'bg-slate-400'}`}></span>
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Calculated from {data.sampleSize} answers</span>
                </div>
              </div>
                  ...
                  <div key={cluster.id} className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm transition-all">
                    <div className="flex items-start gap-4">
                      <div className={`shrink-0 w-16 h-16 rounded-2xl flex flex-col items-center justify-center font-black ${
                        cluster.color === 'blue' ? 'bg-blue-50 text-blue-600' : 
                        cluster.color === 'amber' ? 'bg-amber-50 text-amber-600' : 'bg-rose-50 text-rose-600'
                      }`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-center mb-1">
                          <span className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">Representative Meaning</span>
                          <span className="text-[10px] font-bold text-indigo-400">{Math.round((cluster.count/data.sampleSize)*100)}% Consonance</span>
                        </div>
                        <p className="text-slate-800 font-bold text-lg mb-3 truncate">"{cluster.representative}"</p>
                        
                        <div className="flex flex-wrap gap-2">
                          {cluster.variations.map((v, idx) => (
                            <span key={idx} className="text-[10px] bg-slate-50 text-slate-500 px-2 py-1 rounded-lg border border-slate-100">
                              {v}
                            </span>
                          ))}
                        </div>
              ...
              <div className="mt-8 p-6 bg-slate-900 rounded-[2rem] text-white">
                ...
                <p className="text-[11px] text-slate-400 leading-relaxed">
                  Deep Analysis ($N=100$) samples the model's output space 100 times to provide a high-resolution view of semantic consistency. 
                  Each model architecture has a unique threshold $t \pm \delta$ calibration shown in the selection cards. 
                  Values landing in the highlighted gauge region indicate statistical ambiguity regarding model confidence.
                </p>
              </div>
            </div>
        ...
        {!data && !isAnalyzing && (
          <div className="mt-12 text-center p-12 bg-white rounded-[3rem] border border-dashed border-slate-200">
            <div className="w-16 h-16 bg-indigo-50 rounded-2xl flex items-center justify-center mx-auto mb-6">
               <BrainCircuit className="w-8 h-8 text-indigo-600" />
            </div>
            <h2 className="text-2xl font-black text-slate-800 mb-2">Entropy Extraction Ready</h2>
            <p className="text-slate-400 text-sm max-w-sm mx-auto mb-6">
              Select your analysis depth and enter a query. The model will generate answers (N) to calculate semantic divergence.
            </p>
            <div className="flex justify-center gap-4 text-[10px] font-bold text-slate-300 uppercase tracking-widest">
               <span className="flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> N-Sample Clustering</span>
               <span className="flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> SE Calibration</span>
               <span className="flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> Uncertainty Guarding</span>
            </div>
          </div>
        )}
